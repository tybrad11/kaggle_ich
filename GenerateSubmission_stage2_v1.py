...
# Testing images
test_datapath = '/data/Kaggle/stage_2_images-png-clahe/'
...
seg_weight_dir2 = './'
seg_weight_fname2 = 'Best_Kaggle_Weights_block2d_1024train_v2.h5' #'Best_Kaggle_Weights_resunet_1024train_v3.h5'
seg_weight_filepath2 = join(seg_weight_dir2, seg_weight_fname2)

# Where to save submission output
submission_label = '_stage2_v4'
submission_filepath = 'Submissions/Submission' + submission_label + '.csv'

# Whether to use ensemble

# Whether to use CLAHE normalization in image pre-processing
if 'clahe' in test_datapath.lower():
    use_clahe = False
else:
    use_clahe = True
...
#if too many to fit into memory
if len(img_files) > 1500:
    img_files_batch = []
    for j in range(int(round(len(img_files)/1000))):
        img_files_batch.append(img_files[j*1000:(j+1)*1000])
        
    img_files_batch.append(img_files[(j+1)*1000:])
else:
...
#looping through batches -- for memory purposes
for k in range(len(img_files_batch)):
    if len(img_files_batch) > 100: #if no batches, just all images
        img_files = img_files_batch
    else:
        tqdm.write('Starting image batch ' + str(k) + ' of ' +str(len(img_files_batch)-1))
        img_files = img_files_batch[k]
    ...
    if k == 0:
        ...
        with open(submission_filepath, mode='a', newline='') as f:
            writer = csv.writer(f, delimiter=',')
#            writer.writerow(['ImageId', 'EncodedPixels'])
            for data in submission_data:
                writer.writerow(data)
    ...
    if k == 0:
        ...
        output_dir = 'SampleImagesAndMasks' + submission_label
